...
shoot gamepiece (w/o stopping), go to chargestation */
  ...
  public RedLeftScoreOneBalanceAuto(
    ...
    Translation2d chargeStationMidpoint = new Translation2d(Units.inchesToMeters(36), Units.inchesToMeters(-6));
    Pose2d startPickUpPose = new Pose2d(Units.inchesToMeters(100), 0, Rotation2d.fromDegrees(0));
    Pose2d pickUpPose = new Pose2d(Units.inchesToMeters(190), Units.inchesToMeters(-8), Rotation2d.fromDegrees(0));
    ...
    ParallelCommandGroup retract = new ParallelCommandGroup(
        new ScoreCommand(intake, arm, elevator).withTimeout(1),
    ...
    ParallelDeadlineGroup pickupAndCarryGroup = new ParallelDeadlineGroup(
        new SequentialCommandGroup(
            // Drive to pick up cone
            new ArmOutCommand(arm),
            pickupPath,
            
            // Line up with charge station
            new ArmInCommand(arm),
            lineupPath
        ),
        new IntakeInCommand(intake).beforeStarting(new WaitCommand(1))
    );
